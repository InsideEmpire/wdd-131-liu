...
function ratingTemplate(recipe) {
    let html = `<span class="rating" role="img" aria-label="Rating: ${recipe.rating} out of 5 stars">`;
    for (let i = 1; i <= 5; i++) {
        if (i <= recipe.rating) {
...
function tagTemplate(recipe) {
    ...
    recipe.tags.forEach(tag => {
        html += `<input type="button" value="${tag}">`;
    });
    return html;
}
...
const button = searchDiv.querySelector('img');

function search() {
    main.innerHTML = ``;

    const recipeQuery = input.value;

    console.log(recipeQuery);

    const filteredRecipe = recipes.filter((recipe) => {
        return (
            recipe.name.toLocaleLowerCase().includes(recipeQuery) ||
            recipe.author.toLocaleLowerCase().includes(recipeQuery) ||
            recipe.tags.find((tag) => tag.toLocaleLowerCase().includes(recipeQuery)) ||
            recipe.description.toLocaleLowerCase().includes(recipeQuery)
        )
    });

    const sortedRecipe = filteredRecipe.sort((a, b) => {
        if (a.name < b.name) {
            return -1;
    ...
    sortedRecipe.forEach(renderRecipe);
}
